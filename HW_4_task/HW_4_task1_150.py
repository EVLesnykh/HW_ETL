...
sql.execute("""drop table if exists spark.`HW_4_task_150`""",con)
sql.execute("""CREATE TABLE if not exists spark.`HW_4_task_150` (
...
df1.write.format("jdbc").option("url","jdbc:mysql://localhost:3306/spark?user=root&password=24082019:jhf")\
        .option("driver", "com.mysql.cj.jdbc.Driver").option("dbtable", "HW_4_task_150")\
        .mode("append").save()
df2 = df1.toPandas()
# Get current axis 
ax = plt.gca()
...
df2.plot(kind='line', 
        ...
        color='yellow', ax=ax)
# set the title 
plt.title('Loan Payments Over Tim (150,000 monthly)')
plt.grid ( True )
ax.set(xlabel=None)
# show the plot 
plt.show() 
